...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`users_addresses\` (
  	...
  	\`country\` text DEFAULT 'ایران' NOT NULL,
  	\`phone\` text NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`orders\` (
  	...
  	\`shipping_address_country\` text DEFAULT 'ایران' NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`settings_social_media\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`settings_social_media_order_idx\` ON \`settings_social_media\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`settings_social_media_parent_id_idx\` ON \`settings_social_media\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`settings_footer_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`settings_footer_links_order_idx\` ON \`settings_footer_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`settings_footer_links_parent_id_idx\` ON \`settings_footer_links\` (\`_parent_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`settings_social_media\`;`)
